...
/// A versatile widget that renders the correct UI for any given [ChatMessage].
/// It acts as a router, deciding whether to show a text bubble, a navigation button,
/// or a system message based on the message's properties.
class ChatBubble extends StatelessWidget {
    ...
    switch (message.type) {
      ...
      case MessageType.journalLink:
        return _buildNavigationButton(
          ...
          onPressed: () {
            // This is the intelligent logic to determine which journal to open.
            final dassProvider = context.read<DassResultProvider>();
            JournalType journalType;

            if (dassProvider.results.isEmpty) {
              journalType = JournalType.general;
            } else {
              final latestResult = dassProvider.results.first;
              journalType = _getJournalTypeFromResult(latestResult);
            }

            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (context) => JournalEntryScreen(journalType: journalType),
              ),
            );
          },
        );
  ...
  /// Determines the appropriate journal type based on DASS-21 scores
  /// as per the project guide's specifications.
  JournalType _getJournalTypeFromResult(DassTestRecord result) {
    final isAllNormal = result.depressionSeverity == 'Normal' &&
                        result.anxietySeverity == 'Normal' &&
                        result.stressSeverity == 'Normal';
                        
    final isAllExtreme = result.depressionSeverity == 'Extremely Severe' &&
                         result.anxietySeverity == 'Extremely Severe' &&
                         result.stressSeverity == 'Extremely Severe';

    // Rule #1: If all scores are in the normal range OR all are extremely severe,
    // prompt for a general daily check-in to get a broader perspective.
    if (isAllNormal || isAllExtreme) {
      return JournalType.general;
    }

    // Rule #2: Otherwise, find the highest score to target the most pressing issue.
    final d = result.depressionScore;
    final a = result.anxietyScore;
    final s = result.stressScore;

    if (d >= a && d >= s) {
      return JournalType.depression;
    } else if (a >= d && a >= s) {
      return JournalType.anxiety;
    } else {
      // Default to stress if it's the highest or tied with others.
      return JournalType.stress;
    }
  }

  /// A reusable, styled button for navigating to other parts of the app.
